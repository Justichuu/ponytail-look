...
4. Native platform feature? Use it. World-facing: only after a look.
   JSX is not a look. A date picker is `<input type="date">`.
...
BLIND or UNSEEN. Browser looks need the lamp (bottom, tap, delays — unless
delay is off). Code still greps. Fail closed. If the witness already shows
the native control, delete.

Human and agent learn the same: you only know a look, and you only learn
what you settle.
